...
const AppMenuProfile = () => {
    ...
    const menuProfileActive = useRef(null);

    const hiddenClassName = classNames({ hidden: false });
        ...
        <React.Fragment>
            <div className="layout-menu-profile">
                ...
                <button className="avatar-button p-link" onClick={toggleMenu}>
                    ...
                    <i
                        className={classNames('layout-menu-profile-toggler pi pi-fw pi-angle-down')}
                    ></i>
                </button>

                <ul ref={ulRef} className={classNames('menu-transition')} style={{ overflow: 'hidden', maxHeight: 0, opacity: 0 }}>
                    {menuProfileActive && (
                        <>
                            <li>
